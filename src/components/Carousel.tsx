...
export default function SuggestedCarousel() {
    ...
    const animatedHoverX = useSpring(mouseX, {
        damping: 20, stiffness: 400, mass: 0.1,
    })
    ...
    return (<>
        ...
        <div className="group container mx-6">
            ...
            >
                <motion.div layout className={cn("grid h-full place-items-center rounded-full bg-lime-300", hoverType === "click" && "absolute inset-7 h-auto",)}
                >
                    <motion.span
                        layout="position"
                        className={cn("w-full select-none text-center font-medium uppercase text-gray-900", (hoverType === "prev" || hoverType === "next") && "absolute inset-x-0 top-2", hoverType === "click" && "absolute top-full mt-0.5 w-auto text-sm font-bold text-lime-300",)}
                    >
                        {hoverType ?? "drag"}
                    </motion.span>
                </motion.div>
            ...
            <div className="relative overflow-hidden">
                <motion.ul
                    ...
                    style={{x: animatedX,}}
                    drag="x"
                    dragConstraints={{left: -(FALLBACK_WIDTH * (articles.length - 1)), right: FALLBACK_WIDTH,}}
                ...
                >
                    {articles.map((article, index) => {
                        const active = index === articles.length - 1 - activeSlide
                        return (<motion.li
                            layout
                            key={article.title}
                            ...
                            style={{flexBasis: active ? "40%" : "30%",}}
                        >
                            <Link href={article.url} className="block" target="_blank" rel="noopener noreferrer" draggable={false} onClick={disableDragClick}>
                                <div className={cn("grid place-content-center overflow-hidden rounded-lg bg-gray-900", active ? "aspect-[5/3]" : "aspect-[4/3]",)}>
                                    <span className={cn("text-xl font-bold", active && "text-lime-300",)}> {index} </span>
                                </div>
                            </Link>
