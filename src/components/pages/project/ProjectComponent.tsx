...
function withSrc(images: { title: string; src: string }[]) {
...
export default function ProjectComponent({ project }: Readonly<ProjectProps>) {
    ...
    const lookImages = withSrc(project.lookImages);
    const currentImages = withSrc(project.currentImages);
    return (
        <>
            <ScrollProgress />
            <section className="w-full p-1 md:p-6 lg:p-14">

                <MotionUpDiv className="mt-12 md:mt-8 lg:mt-6"><AppTitle text={titleText} prefix={titlePrefix} /></MotionUpDiv>
                <div className="h-24" />
                ...
                <div className="flex flex-col justify-center items-center w-full">
                    ...
                    {lookImages.length > 0 && (
                        <>
                            ...
                            <MotionUpDiv className="w-full flex flex-col justify-center items-center" viewportAmount={0.2}>
                                <NormalImageGallery images={lookImages} fit="contain" />
                            </MotionUpDiv>
                    ...
                    {currentImages.length > 0 && (
                        <>
                            ...
                            <MotionUpDiv className="w-full flex flex-col justify-center items-center" viewportAmount={0.2}>
                                <div className="h-8" />
                                <NormalImageGallery images={currentImages} fit="cover" />
                            </MotionUpDiv>
                        </>
                    )}
                    ...
                    <MotionUpDiv className="w-full" viewportAmount={0.2}>
                        <iframe title={"موقع المشروع"} src={project.map.src} width="100%" height="600" style={{ border: 0 }}
